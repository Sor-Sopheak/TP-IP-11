...
export class ProductService {
  ...
  async findAll(): Promise<Product[]> {
   ...
   async findCategoryItems(subCategoryId = '') {
    ...
    // if (categoryId) matchCond['categoryId'] = new ObjectId(categoryId);
    // if (subCategoryId) matchCond['subCategoryId'] = new ObjectId(subCategoryId);
  ...
  async updateById(
    ...
    const { id, name, description, imageUrl, categoryId, subCategoryId, price } =
      product;
    if (subCategoryId && categoryId) {
			const subctgIds = await this.subctgModel.find({}, '_id').exec();
			for (const scId of subctgIds) {
				if (scId._id.toString() === subCategoryId) {
					const updateProduct = await this.productModel.findByIdAndUpdate(id, {name, description, imageUrl, categoryId, subCategoryId, price }, {new: true});
					return {
						status: true,
